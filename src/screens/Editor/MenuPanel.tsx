...
import { addRoom, addDoor, show3D, addFloor, switchFloor, addWindow } from "@stores/app/action";
import { Icon3D, IconBuilding, IconRoom, IconSave, IconWindow } from "@assets/icons";
...
const MenuPanel: React.FC<Props> = (props) => {
  ...
  const construction = useSelector((state: rootState) => state.app.construction);
    ...
    <>
      {state.modal && (
        <Modal title='Add Room'>
          <InputField placeholder='Name' value={state.form.name} onChange={(v) => handleChange(v, "name")} />
          ...
          <View style={css("flex flex-col mt-4")}>
            <PickerIOS
              ...
              onValueChange={(itemValue) => handleWallPickerChange(itemValue, "construction")}
            >
              {construction.map(
                (wall, index) =>
                  wall.category === "wall" && <PickerIOS.Item label={wall.name} value={wall.name} key={index} />
              )}
            </PickerIOS>
          </View>
          <View style={css("flex flex-col mt-4")}>
            <PickerIOS
              ...
              onValueChange={(itemValue) =>
                handlePickerChange({ ...state.form.construction, floor: itemValue }, "construction")
              }
            >
              {construction.map(
                (floor, index) =>
                  floor.category === "floor" && <PickerIOS.Item label={floor.name} value={floor.name} key={index} />
              )}
            </PickerIOS>
          </View>
          <View style={css("flex flex-col mt-4")}>
            <PickerIOS
              ...
              style={{ height: 50, width: "100%", marginTop: 0 }}
              onValueChange={(itemValue) =>
                handlePickerChange({ ...state.form.construction, roof: itemValue }, "construction")
              }
            >
              {construction.map(
                (roof: Construction, index: number) =>
                  roof.category === "roof" && <PickerIOS.Item label={roof.name} value={roof.name} key={index} />
              )}
            </PickerIOS>
          </View>
      ...
      <View style={css("h-20 w-full px-4 flex-row items-center justify-between")}>
        <ScrollView horizontal contentContainerStyle={css("flex-row w-2/3")}>
          ...
          <Button text='+ Floor' style={{ width: 100 }} onPress={() => addNewFloor()} />
        </ScrollView>
